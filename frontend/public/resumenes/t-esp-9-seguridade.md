...
```mermaid
graph TD
    A["Seguridade Operativa"] --> B["Formación e Adestramento"]
    A --> C["Protocolos Estándar (OACEL)"]
    A --> D["Equipo de Protección (EPI)"]
    B --- E["Coñecemento do comportado do lume"]
    C --- F["10 Normas e 13 Situacións de Risco"]
    D --- G["Mantemento e uso obrigatorio"]
```
...
```mermaid
sequenceDiagram
    participant B as Banco (40cm H / 33cm M)
    participant T as Tempo (5 Minutos)
    participant R as Recuperación (15 seg)
    
    Note over B,T: Ritmo: 90 movementos por minuto
    B->>T: Inicio do exercicio
    T->>R: Fin do exercicio e descanso
    R->>R: Medición do pulso (Coeficiente de Saúde)
```
